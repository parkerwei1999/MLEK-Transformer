...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        self.proj = QConv2d(in_chans,
                            ...
                            quantizer_str=cfg.QUANTIZER_W)
        if norm_layer:
            # Match upstream FQ-ViT for the PTF path: PatchEmbed uses the generic
            # layer-wise activation grid even though its output feeds LayerNorm.
            # Keep the fork-only PCS path channel-wise for APO equivalence.
            if cfg.INT_NORM:
                patch_calibration_mode = cfg.CALIBRATION_MODE_A
                patch_observer = cfg.OBSERVER_A
                patch_quantizer = cfg.QUANTIZER_A
            else:
                patch_calibration_mode = cfg.CALIBRATION_MODE_A_LN
                patch_observer = cfg.OBSERVER_A_LN
                patch_quantizer = cfg.QUANTIZER_A_LN
            self.qact_before_norm = QAct(
                quant=quant,
                calibrate=calibrate,
                bit_type=cfg.BIT_TYPE_A,
                calibration_mode=patch_calibration_mode,
                observer_str=patch_observer,
                quantizer_str=patch_quantizer)
