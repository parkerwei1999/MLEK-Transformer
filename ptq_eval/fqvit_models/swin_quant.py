...
class SwinTransformer(nn.Module):
    ...
    def model_quant(self):
        # In linear-only mode, QActs remain enabled only when their boundary
        # feeds a quantized Linear/Conv. Boundaries owned by LayerNorm, Softmax,
        # residual hand-off, or patch-merge hand-off stay in fp32.
        linear_only = getattr(self.cfg, "LINEAR_ONLY", False)
        disable_patterns = [
            re.compile(r"^patch_embed\.qact(?:_before_norm)?$"),
            re.compile(r"^qact[12]$"),
            re.compile(r"^layers\.\d+\.blocks\.\d+\.qact[24]$"),
            re.compile(
                r"^layers\.\d+\.blocks\.\d+\.attn\."
                r"(?:qact_attn1|qact_table|qact2)$"),
            re.compile(r"^layers\.\d+\.downsample\.qact2$"),
        ] if linear_only else []
        ...
        for name, m in self.named_modules():
            if type(m) in [QConv2d, QLinear]:
                ...
                m.quant = not any(p.match(name) for p in disable_patterns)
            elif type(m) is QIntSoftmax:
                m.quant = not linear_only
            if self.cfg.INT_NORM and type(m) is QIntLayerNorm:
                m.mode = "int"
    ...
    def forward_features(self, x):
        ...
        if self.absolute_pos_embed is not None:
            if hasattr(self, "apo_positional_add"):
                x = self.apo_positional_add(x, self.absolute_pos_embed)
            else:
        ...
        for i, layer in enumerate(self.layers):
            if i == 0:
                last_quantizer = (
                    ...
                    if self.absolute_pos_embed is not None
                    else self.patch_embed.qact.quantizer)
            else:
                last_quantizer = self.layers[i - 1].downsample.qact2.quantizer
...
_SWIN_PRETRAINED_URLS = {
    "tiny": (
        "https://github.com/SwinTransformer/storage/releases/download/v1.0.8/"
        "swin_tiny_patch4_window7_224_22kto1k_finetune.pth"
    ),
    "small": (
        "https://github.com/SwinTransformer/storage/releases/download/v1.0.8/"
        "swin_small_patch4_window7_224_22kto1k_finetune.pth"
    ),
    "base": (
...
def _load_swin_pretrained(model, size):
    checkpoint = torch.hub.load_state_dict_from_url(
        url=_SWIN_PRETRAINED_URLS[size],
        map_location="cpu",
        check_hash=True,
    )
...
def swin_tiny_patch4_window7_224(pretrained=False,
                                 ...
                                 cfg=None,
                                 **kwargs):
    """Swin-T @ 224x224, pretrained on ImageNet-22k and fine-tuned on ImageNet-1k."""
    model = SwinTransformer(patch_size=4,
                            ...
                            **kwargs)
    if pretrained:
        _load_swin_pretrained(model, "tiny")
    return model
...
def swin_small_patch4_window7_224(pretrained=False,
                                  ...
                                  cfg=None,
                                  **kwargs):
    """Swin-S @ 224x224, pretrained on ImageNet-22k and fine-tuned on ImageNet-1k."""
    model = SwinTransformer(patch_size=4,
                            window_size=7,
                            embed_dim=96,
                            depths=(2, 2, 18, 2),
                            ...
                            **kwargs)
    if pretrained:
        _load_swin_pretrained(model, "small")
    return model
...
def swin_base_patch4_window7_224(pretrained=False,
                                 ...
                                 cfg=None,
                                 **kwargs):
    """Swin-B @ 224x224, pretrained on ImageNet-22k and fine-tuned on ImageNet-1k."""
    model = SwinTransformer(patch_size=4,
                            window_size=7,
                            embed_dim=128,
                            depths=(2, 2, 18, 2),
                            ...
                            **kwargs)
    if pretrained:
        _load_swin_pretrained(model, "base")
    return model
