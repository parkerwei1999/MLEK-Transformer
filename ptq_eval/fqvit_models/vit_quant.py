...
class VisionTransformer(nn.Module):
    ...
    def model_quant(self):
        # `linear_only` disables QActs that sit at a non-linear op's INPUT
        # (LN input, Softmax input). Output-side QActs (feeding next Linear)
        # stay on. Pattern: `dq → fp_nonlinear → q` boundary.
        # Named modules whose leaf name or full path matches any of these
        # get m.quant=False in linear_only; everything else stays on.
        linear_only = getattr(self.cfg, 'LINEAR_ONLY', False)
        _DISABLE_PATTERNS = [
            re.compile(r'^qact1$'),                            # outer: feeds first block LN1
            re.compile(r'^blocks\.\d+\.qact2$'),               # attn_out+residual → LN2 input
            re.compile(r'^blocks\.\d+\.qact4$'),               # mlp_out+residual → next LN1 input
            re.compile(r'^blocks\.\d+\.attn\.qact_attn1$'),    # attention softmax input
        ] if linear_only else []
        ...
        for name, m in self.named_modules():
            if type(m) in [QConv2d, QLinear]:
                ...
                disabled = any(p.match(name) for p in _DISABLE_PATTERNS)
                m.quant = not disabled
    ...
    def forward_features(self, x):
        ...
        x = self.qact_embed(x)
        positional = self.qact_pos(self.pos_embed)
        if hasattr(self, "apo_positional_add"):
            x = self.apo_positional_add(x, positional)
        else:
            x = x + positional
        x = self.qact1(x)
