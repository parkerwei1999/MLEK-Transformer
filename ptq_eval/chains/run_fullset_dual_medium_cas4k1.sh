#!/usr/bin/env bash
# whisper-medium on the full LibriSpeech test-clean (2620 utt): fp32 reference and the
...
run medium fp32_static --stages fp32-static
run medium v4r16_dualk8 --stages int8 --prec-rules "$INT8;$LNR" --ln-dual-k 8 --ln-newton-steps 0
echo "== done $(date +%T)"
