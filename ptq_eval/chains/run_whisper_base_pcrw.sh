...
run() { echo "== whisper-base $1 $(date +%T)"
  "$PY" whisper_ptq_eval.py --model-id openai/whisper-base --out-dir "$OUT/pcrw/wb_$1" --stages int8 --n-cal 200 --n-eval 200 --quant-config a16w8 --act-observer minmax --prec-rules "$INT8;layers\\.\\d+\$=a16inpc8symout@add;$LN32" --ln-newton-steps 1 $2 2>&1 | grep -E "pc_rewrite|200/200|Traceback|Error"; }
