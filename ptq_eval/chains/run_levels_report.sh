...
echo "== census whisper-large-v3 $(date +%T)"; "$PY" ln_var_profile.py --model-id openai/whisper-large-v3 --n 8 2>&1 | grep -E "^===|^ *[0-9]|levels_med|LNs with|Traceback|Error|out of memory" | cut -c1-150
echo "== done $(date +%T)"
