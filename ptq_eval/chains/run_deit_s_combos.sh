#!/usr/bin/env bash
# DeiT-Small: LN int16 plus one more family at int16; waits for the DeiT-B chain.
set -uo pipefail
...
for extra in 'mlp\.fc2$=a16w8' 'mlp\.fc1$=a16w8' 'blocks\.\d+$=a16w8' 'attn(\.log_int_softmax|\.attn_drop)?$=a16w8' '(fc1|fc2|qkv|proj)$=a16w8'; do
  ...
  echo "== deit_small LN16 + [$extra] $(date +%T)"
  "$PY" vision_ptq_eval.py --out-dir "$V/et131_deit_small_combo_$i" --model-name deit_small_patch16_224 \
